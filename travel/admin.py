...
class AnalysisToolAdmin(admin.ModelAdmin):
    ...
    # Change 권한만 남기고 리스트 노출
    def get_model_perms(self, request):
        return {"change": True}

    # 리스트 클릭시 run_view로 리다이렉트
    def changelist_view(self, request, extra_context=None):
        return redirect("travel:llm_analysis")

    # 커스텀 URL 등록 (run_view만)
    def get_urls(self):
        urls = super().get_urls()
    ...
    def run_view(self, request):
        ...
        if request.method == "GET":
            ...
            grouped_places = {
                cat: list(items) for cat, items in groupby(all_places, key=attrgetter("category"))
            }
    ...
    # -------------------
    # save_model: PlaceAnalysis 저장
    # -------------------
    def save_model(self, request, obj, form, change):
        """
        obj: AnalysisTool 객체 (Admin에서 단일 객체)
        form.cleaned_data: POST에서 넘어온 데이터를 활용
        """
        # POST에서 payload_* 키를 모두 읽어 분석 저장
        saved = 0
        errors = 0
